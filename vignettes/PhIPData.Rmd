...
* `logfc`: matrix of log10 estimated fold-changes (in comparison to negative control samples),
...
counts_dat <- matrix(runif(n_samples*n_peps, min = 1, max = 1e6), 
                     nrow = n_peps)
...
sample_meta <- data.frame(sample_name = paste0("sample", 1:n_samples),
                          gender = sample(c("M", "F"), n_samples, T),
                          group = sample(c("ctrl", "trt"), n_samples, T))
...
assays(phip_obj)$counts   # Returns the same as assays(phip_obj)[["counts"]]
```

While `assays(phip_obj)` returns a list of all assays in the `PhIPData` object, `assay(phip_obj, i)` returns a `DataFrame` of the specified assay. If `i` is missing, `assay(phip_obj)` defaults to the first assay (`counts`). `i` can be a character specifying the assay name or a numeric index of the assay.

```{r}
assay(phip_obj, "logfc")   # Returns the same as assay(phip_obj, 2)
```
...
After a `PhIPData` object has been created, data for new and existing assays can be set using `<-`. Dimension names of the replacement assays are automatically corrected to be identical to the object names. Replacement assays must also be on the same dimension of the existing object. 
...
```{r}
phip_obj[, phip_obj$group == "trt"]
```

In addition to subsetting by row indices, `PhIPData` supports subsetting rows using peptide metadata information. Using syntax similar to `dplyr::filter()`, `subsetByPeptideMeta(phip_obj, condition)` returns a `PhIPData` object with rows where the specified condition holds. 

```{r}
ebv_sub <- subsetByPeptideMeta(phip_obj, 
                               grepl("Epstein-Barr virus", species))
...
# Use alias ----------
hiv_sub <- subsetByPeptideMeta(phip_obj, grepl(getAlias("hiv"), species))
...
subsetByPeptideMeta(phip_obj, grepl(getAlias("hiv"), species))
```
