...
virscan_info <- readr::read_tsv("../testdata/VirScan_annotation.tsv",
  ...
  as.data.frame() %>%
  dplyr::rename(species = taxon_species)
...
test_that("PhIPData objects can be subsetted by virus", {
  ...
  peptideInfo(phip_obj) <- virscan_info %>% dplyr::rename(taxonomy_species = species)
  expect_error(getPeptides(phip_obj, alias$pattern[1]), "Peptide metadata does not contain `species` information.")
})
